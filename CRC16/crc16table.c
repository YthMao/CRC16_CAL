...
unsigned short crc16_ibm_table[256] ={
...
unsigned short crc16_xmodem_table[256] ={
...
unsigned short CRC16_CCITT(unsigned char *pData, unsigned int nLength)  
{  
...
unsigned short CRC16_CCITT_FALSE(unsigned char *pData, unsigned int nLength)  
{  
    ...
    {
        temp = fcs >> 8;
        fcs = (fcs << 8) ^ crc16_xmodem_table[(temp ^ *pData++) & 0xffff];
    }
    return fcs;
}  
unsigned short CRC16_XMODEM(unsigned char *pData, unsigned int nLength)  
{  
    unsigned short fcs = 0x0000;    // 初始化
    unsigned char  temp;
    while(nLength--)
    {
        temp = fcs >> 8;
        fcs = (fcs << 8) ^ crc16_xmodem_table[(temp ^ *pData++) & 0xff];
    }
    return fcs;
}  
...
unsigned short CRC16_MODBUS(unsigned char *pData, unsigned int nLength)  
{  
    unsigned short fcs = 0xFFFF;    // 初始化
    while(nLength--)
    {
        fcs = (fcs >> 8) ^ crc16_ibm_table[(fcs ^ *pData++) & 0xff];
    }
    return fcs;
}  
unsigned short CRC16_IBM(unsigned char *pData, unsigned int nLength)  
{  
    unsigned short fcs = 0x0000;    // 初始化
    while(nLength--)
    {
...
unsigned short CRC16_MAXIM(unsigned char *pData, unsigned int nLength)  
{  
    unsigned short fcs = 0x0000;    // 初始化
    while(nLength--)
    {
